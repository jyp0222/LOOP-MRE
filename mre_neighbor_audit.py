...
def _load_records(run_dir):
    ...
    if manifest.get('protocol') != 'mre_transductive_fixed_base_novel':
        raise ValueError('Expected a fixed-base/novel MRE manifest')
...
def audit_run(run_dir):
    """Read one original run and return a report without writing inputs."""
    ...
    return {
        ...
        'run_dir': str(run_dir), 'seed': manifest.get('seed'),
