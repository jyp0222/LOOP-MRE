...
class NeighborAuditTests(unittest.TestCase):
    ...
    def test_hand_counted_metrics_and_conditional_denominators(self):
        ...
        self.assertEqual(summary["protocol"], "mre_transductive_random_half")
        self.assertEqual((summary["n_base"], summary["n_total"]), (2, 4))
    ...
    def test_legacy_fixed_protocol_is_still_supported(self):
        self.manifest["protocol"] = "mre_transductive_fixed_base_novel"
        self.write_manifest()
        report = audit_run(self.run_dir)
        self.assertEqual(report["protocol"], "mre_transductive_fixed_base_novel")
        self.assertEqual(report["scopes"]["all_decisions"]["overall"]["decision_count"], 9)

    def test_random_half_rejects_inconsistent_class_counts(self):
        self.manifest["n_base"] = 1
        self.manifest["base_classes"] = self.manifest["classes"][:1]
        self.manifest["novel_classes"] = self.manifest["classes"][1:]
        self.write_manifest()
        with self.assertRaisesRegex(ValueError, "Random-half"):
            audit_run(self.run_dir)

    def test_audits_actual_random_half_prepared_files(self):
        source = self.run_dir / "source"
        source.mkdir()
        for name, relations in (("train", ("alpha", "beta")),
                                ("test", ("gamma", "delta"))):
            rows = [
                {"relation": relation, "tokens": ["head", "visits", "tail", str(index)],
                 "h": {"pos": [0]}, "t": {"pos": [2]}}
                for relation in relations for index in range(6)
            ]
            (source / (name + ".txt")).write_text(
                "".join(repr(row) + "\n" for row in rows), encoding="utf-8")
        actual_run = self.run_dir / "actual_run"
        manifest = prepare_dataset(source, actual_run / "data", seed=2, expected_total=4)
        self.assertEqual(manifest["protocol"], "mre_transductive_random_half")
        train = [json.loads(line) for line in
                 (actual_run / "data" / "train_labeled.jsonl").read_text(encoding="utf-8").splitlines()]
        test = [json.loads(line) for line in
                (actual_run / "data" / "test.jsonl").read_text(encoding="utf-8").splitlines()]
        decisions = [self.decision(test[0]["id"],
                                   [train[0]["id"], test[1]["id"]], test[1]["id"])]
        (actual_run / "neighbor_queries.jsonl").write_text(
            json.dumps(decisions[0]) + "\n", encoding="utf-8")
        report = audit_run(actual_run)
        self.assertEqual((report["protocol"], report["n_base"], report["n_total"]),
                         ("mre_transductive_random_half", 2, 4))
        self.assertEqual(report["scopes"]["all_decisions"]["overall"]["decision_count"], 1)
